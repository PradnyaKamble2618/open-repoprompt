...
// rebuildUILocked rebuilds the UI based on the current files.
// Caller must hold t.mu.
func (t *FileTreeWidget) rebuildUILocked() {
	// Clear the container
...
func (t *FileTreeWidget) addFileToUI(file *fileutils.FileInfo, indent int) {
	// Capture file in a local variable for the closure
	f := file
	...
	check.OnChanged = func(checked bool) {
		...
		if f.IsDir {
			...
			if checked && len(f.Children) == 0 {
				t.loadChildrenLocked(f)
			}

			// Select/unselect all children
			t.toggleSelectionLocked(f, checked)
		}
		t.mu.Unlock()
...
// loadChildrenLocked loads children for a directory.
// Caller must hold t.mu.
func (t *FileTreeWidget) loadChildrenLocked(dir *fileutils.FileInfo) {
...
// toggleSelectionLocked toggles the selection state of a file and its children.
// Caller must hold t.mu.
func (t *FileTreeWidget) toggleSelectionLocked(file *fileutils.FileInfo, selected bool) {
	...
	if file.IsDir {
		...
		for _, child := range file.Children {
			t.toggleSelectionLocked(child, selected)
		}
	}
}
